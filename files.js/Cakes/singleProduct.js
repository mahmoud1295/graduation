...
function addData(obj, quantity){
    ...
    set(ref(db, `user/${username}/cakeProducts/` + obj.idCart), {
        name: obj.name,
        id: obj.idCart,
        img: obj.img,
        price: obj.curPrice,
...
fetch('https://login1form-3fad9-default-rtdb.firebaseio.com/singleProduct.json').then((res)=>{
    let data = res.json();
    return data;
}).then((res)=>{
    obj = res;
    ...
    fetch('https://login1form-3fad9-default-rtdb.firebaseio.com/CurrentUser.json').then((user)=>{
        ...
        // console.log(obj)
        fetch(`https://login1form-3fad9-default-rtdb.firebaseio.com/user/${username}/cakeProducts/${obj.idCart}/quantity.json`).then((res)=>{
            let data = res.json();
            return data;
        }).then((res)=>{
            quantity = res;
            ...
            addCart.addEventListener('click', ()=>{
                if (quantity) {
                    ...
                    alert('the product has been remove from the cart');
                }
            })
        })
    })
...
function deleteData (id){
    remove(ref(db, `user/${username}/cakeProducts/` + id)).catch(()=>{
        console.log(Error);
    });
}