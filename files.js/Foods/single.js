...
function addData(obj, quantity){
    ...
    set(ref(db, `user/${username}/products/` + obj.idCart), {
        name: obj.name,
        id: obj.idCart,
        img: obj.img,
        price: obj.curPrice,
...
fetch('https://login1form-3fad9-default-rtdb.firebaseio.com/singleProductfood.json').then((res)=>{
    let data = res.json();
    return data;
}).then((res)=>{
    obj = res;
    ...
    fetch('https://login1form-3fad9-default-rtdb.firebaseio.com/CurrentUser.json').then((user)=>{
        ...
        // console.log(obj)
        fetch(`https://login1form-3fad9-default-rtdb.firebaseio.com/user/${username}/products/${obj.idCart}/quantity.json`).then((res)=>{
            let data = res.json();
            return data;
        }).then((res)=>{
            quantity = res;
...
function deleteData (id){
    remove(ref(db, `user/${username}/products/` + id)).catch(()=>{
        console.log(Error);
    });
}